...
We teach Kubernetes first; we then teach what GKE adds. Workload Identity, Autopilot, GKE-specific autoscaling profiles — these are the *additions* to a foundation the learner already owns. If GKE were to disappear tomorrow, a C18 graduate could run the same workload on EKS, AKS, or self-hosted k0s.
...
We teach BigQuery as a distinctive product *and* benchmark it against Trino + Iceberg every time. Learners must know the open-source equivalent's performance characteristics by Week 10.

We teach Pub/Sub as a distinctive product *and* benchmark it against Kafka, NATS, and Redpanda. Learners must defend the choice on operational complexity, not on familiarity.
