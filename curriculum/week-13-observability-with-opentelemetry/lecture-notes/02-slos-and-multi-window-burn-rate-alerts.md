...
```mermaid
stateDiagram-v2
  Normal --> HeadsUp: budget over 50 percent
  HeadsUp --> SlowDown: budget over 75 percent
  SlowDown --> Freeze: budget over 90 percent
  Freeze --> IncidentReview: budget at 100 percent
  IncidentReview --> Normal: budget resets next window
```
*A team's posture escalates as the 28-day error budget burns down, then resets with the rolling window.*
...
```mermaid
flowchart TD
  A["Error budget burn rate"] --> B{"Long window over threshold"}
  B -->|No| Z["No alert"]
  B -->|Yes| C{"Short window also over threshold"}
  C -->|No, already recovered| Z
  C -->|Yes, still burning| D{"Which burn rate"}
  D -->|"14.4x"| E["Page fast burn"]
  D -->|"6x"| F["Page medium burn"]
  D -->|"1x"| G["Ticket slow burn"]
```
*The long window sets severity; the short window must still agree before anything fires.*
