...
```mermaid
flowchart TD
  A["Packet arrives for a destination"] --> B["Find all matching routes"]
  B --> C{"More than one route matches"}
  C -- No --> D["Use that route"]
  C -- Yes --> E{"Does one route have a longer prefix"}
  E -- Yes --> F["Longest prefix wins"]
  E -- No --> G["Prefixes tie"]
  G --> H["Lowest priority number wins"]
```
*GCP resolves competing routes by longest prefix first, then by lowest priority number.*
...
```mermaid
flowchart LR
  A["Private VM no external IP"] --> B["Cloud Router"]
  B --> C["Cloud NAT"]
  C --> D["Internet"]
```
*Cloud NAT rides on a Cloud Router to give a VM with no external IP outbound-only internet access.*
