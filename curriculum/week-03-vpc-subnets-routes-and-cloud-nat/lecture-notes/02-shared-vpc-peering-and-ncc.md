...
flowchart LR
  ...
  C -.-> A
```
*All three VPCs sit as spokes on one NCC hub, so A can reach C even though neither peers directly with the other.*
...
```mermaid
flowchart TD
  A["Need to connect VPCs"] --> B{"One team should own the network for everyone"}
  B -- Yes --> C["Shared VPC"]
  B -- No --> D{"How many networks and is transitivity needed"}
  D -- "Two or three no transitivity" --> E["Peering"]
  D -- "Four or more or transitivity needed" --> F["Network Connectivity Center"]
```
*The two-question shortcut for choosing shared VPC, peering, or NCC.*
