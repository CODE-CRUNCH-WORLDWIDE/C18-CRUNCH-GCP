...
```mermaid
flowchart TD
  A["Organization - root of the tree"] --> B["Folder - optional grouping node"]
  B --> C["Project - the isolation unit"]
  C --> D["Resource - VM or bucket or topic"]
```
*IAM and org policy inherit downward from organization to resource; only projects hold resources.*
...
```mermaid
flowchart TD
  A["Bootstrap runs with local state"] --> B["Create bootstrap folder tf-state project and GCS bucket"]
  B --> C["Migrate state into the new bucket"]
  C --> D["Shared and workloads layers use the GCS backend from day one"]
```
*How Terraform bootstraps its own remote state before any other layer can depend on it.*
