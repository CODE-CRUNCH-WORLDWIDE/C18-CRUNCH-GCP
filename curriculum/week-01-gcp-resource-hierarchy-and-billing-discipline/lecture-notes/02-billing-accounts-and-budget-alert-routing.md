...
```mermaid
erDiagram
  BILLING_ACCOUNT ||--o{ PROJECT : "linked to"
```
*One billing account can link to many projects, but each project links to exactly one billing account at a time.*
...
```mermaid
flowchart LR
  A["Budget threshold crossed"] --> B["Pub Sub topic billing-alerts"]
  B --> C["Cloud Function budget-to-slack"]
  C --> D["Slack channel gcp-cost"]
```
*The programmatic notification path that turns a budget crossing into a Slack message instead of an unread email.*
