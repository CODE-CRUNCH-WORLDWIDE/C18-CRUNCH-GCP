...
```mermaid
flowchart TD
  A["Enable audit logs"] --> B["Apply the four deny constraints"]
  B --> C["Add CMEK and Secret Manager"]
  C --> D["VPC Service Controls dry-run"]
  D --> E["Fix ingress rules from the log"]
  E --> F["VPC Service Controls enforce"]
  F --> G["Binary Authorization dry-run"]
  G --> H["Binary Authorization enforce"]
```
*The day-one rollout order: additive and safe controls first, restrictive controls last and always through dry-run before enforce.*
...
```mermaid
flowchart LR
  In["Request from inside the perimeter"] --> Svc["Protected service such as BigQuery"]
  Out["Request from outside the perimeter"] --> Check{"Ingress rule matches the identity"}
  Check -->|"No match"| Deny["Denied with a 403"]
  Check -->|"Matches, e.g. the Cloud Build service account"| Svc
```
*A service perimeter allows requests from inside, and only lets outside identities in when an explicit ingress rule names them.*
