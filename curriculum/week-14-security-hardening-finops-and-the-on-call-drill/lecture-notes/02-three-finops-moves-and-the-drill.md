...
```mermaid
flowchart TD
  A["Page fires from a burn-rate alert"] --> B["Acknowledge and triage"]
  B --> C["Mitigate: rollback, failover, scale, or shed load"]
  C --> D["Confirm recovery holds"]
  D --> E["Write the no-blame postmortem"]
```
*The on-call loop: mitigate before you diagnose, and close every page with a postmortem.*
...
```mermaid
flowchart TD
  Q1{"Is this real"} -->|"No, monitoring artifact"| FixAlert["Fix the alert, not the service"]
  Q1 -->|"Yes, users affected"| Q2{"What changed"}
  Q2 --> Deploy["Recent deploy: roll back"]
  Q2 --> Traffic["Traffic spike: scale or rate-limit"]
  Q2 --> Dep["Dependency outage: fail over"]
  Q2 --> Limit["Resource or quota limit: check the logs"]
```
*Triage decision tree: confirm real user impact first, then check the four usual suspects in order.*
