...
```mermaid
flowchart LR
  Price["Price"] -->|"batching raises tail latency"| Latency["Latency"]
  Latency -->|"regaining sovereignty adds cold starts"| Sovereignty["Sovereignty"]
  Sovereignty -->|"idle private GPUs cost more"| Price
```
*Each corner of the triangle is bought at the expense of one of the other two.*
...
```mermaid
flowchart TD
  Req["Incoming request"] --> B{"Which bargain?"}
  B --> Gemini["Gemini API"]
  B --> Vertex["Vertex Endpoint"]
  B --> VLLM["vLLM on GKE"]
  Gemini --> G1["Network plus shared fleet queue"]
  Vertex --> V1["Network plus replica queue"]
  Vertex --> V2["Cold start on scale up"]
  VLLM --> L1["Network plus batching queue"]
  VLLM --> L2["Spot preemption cliff"]
```
*Each bargain's p99 tail traces back to a different structural cause.*
