...
```mermaid
flowchart TD
  A["Workload requirements"] --> B{"Sovereignty constraint?"}
  B -->|"PII must stay in VPC"| C["Eliminate public Gemini API"]
  B -->|"No constraint"| D["All three bargains stay in play"]
  C --> E{"Traffic shape?"}
  D --> E
  E -->|"Spiky near zero"| F["Favors Gemini API"]
  E -->|"Steady high duty cycle"| G["Favors Vertex Endpoint"]
  E -->|"Steady very high volume"| H["Favors vLLM on GKE"]
  F --> I["Volume and control tiebreaker"]
  G --> I
  H --> I
  I --> J["Final bargain decision"]
```
*Sovereignty, then traffic shape, then volume-and-control narrow the three bargains down to one decision.*
...
```mermaid
flowchart LR
  Model1["Model artifact v1"] --> DM1["DeployedModel v1"]
  Model2["Model artifact v2 canary"] --> DM2["DeployedModel v2"]
  DM1 --> Endpoint["Endpoint stable URL"]
  DM2 --> Endpoint
  DM1 --> Replica1["Replica VM with GPU"]
  DM2 --> Replica2["Replica VM with GPU"]
  Endpoint -->|"traffic split percent"| Client["Incoming request"]
```
*A Model becomes billable only once bound to an Endpoint as a DeployedModel backed by GPU replicas; traffic split routes requests across more than one.*
