...
```mermaid
flowchart TD
  A["VM beats container platform when"] --> B["Legacy stateful daemon needs stable disk and identity"]
  A --> C["GPU batch wants the whole accelerated machine"]
  A --> D["Sovereignty residency or licensing constraint"]
```
*The three category triggers that override the container-first default.*
...
```mermaid
flowchart LR
  A["Lift and shift rehost onto GCE VM"] --> B["Replatform swap DB LB and cache to managed services"]
  B --> C["Refactor containerize onto GKE or Cloud Run"]
```
*The three-phase migration path, with the VM as the landing zone at every stage.*
