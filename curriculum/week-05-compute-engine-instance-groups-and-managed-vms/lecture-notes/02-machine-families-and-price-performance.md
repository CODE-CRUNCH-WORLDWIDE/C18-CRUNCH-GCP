...
```mermaid
flowchart TD
  A["Stateless Go HTTP service CPU bound"] --> B["General purpose workload"]
  B --> C["Scale out and throughput oriented"]
  C --> D["Lean T2D full core price performance"]
  D --> E["p99 SLO loose so skip C3 premium"]
  E --> F["Buy CUD on baseline run peak on demand"]
```
*Walking the five-step machine-family decision for this week's stateless Go service.*
...
```mermaid
flowchart LR
  A["Launch e2 n2d and t2d instances"] --> B["Deploy identical Go binary"]
  B --> C["Load test each with hey"]
  C --> D["Compute RPS per dollar"]
  D --> E["Tear down all instances"]
```
*The benchmark pipeline that turns a price-performance claim into a measured number.*
