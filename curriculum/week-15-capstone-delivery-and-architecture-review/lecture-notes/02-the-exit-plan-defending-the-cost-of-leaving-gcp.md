...
```mermaid
flowchart LR
  G["Green - commodity"] --> Y["Yellow - portable model proprietary runtime"]
  Y --> O["Orange - proprietary with OSS analog"]
  O --> R["Red - hard to replace at parity"]
  G -.-> G1["GKE and Cloud SQL Postgres"]
  Y -.-> Y1["Dataflow Beam and Cloud Run"]
  O -.-> O1["BigQuery and Pub Sub"]
  R -.-> R1["Spanner external consistency"]
```
*The portability spectrum, with a capstone example dependency at each tier.*
...
classDiagram
  ...
  Handler --> StateRepository
  StateRepository <|.. SpannerRepository
  StateRepository <|.. CockroachRepository
```
*The repository interface seam - business logic depends on StateRepository, not on Spanner directly, so a new implementation is a self-contained swap.*
