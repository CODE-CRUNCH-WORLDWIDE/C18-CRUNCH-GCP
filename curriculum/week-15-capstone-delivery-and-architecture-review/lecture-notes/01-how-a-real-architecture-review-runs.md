...
```mermaid
flowchart LR
  A["Min 0 to 5 - Context"] --> B["Min 5 to 15 - Diagram walk"]
  B --> C["Min 15 to 30 - Trace one event"]
  C --> D["Min 30 to 45 - Failure modes and blast radius"]
  D --> E["Min 45 to 55 - Cost and exit"]
  E --> F["Min 55 to 60 - Risk list and sign-off"]
```
*The sixty-minute review agenda, in order.*
...
```mermaid
flowchart TD
  Page["Paged - ingest SLO fast burn"] --> Dash["Open SLO dashboard - check golden signals"]
  Dash --> Region{"Is this a region event?"}
  Region -->|Yes| Wait["Confirm standby is serving then wait - failover is automatic"]
  Region -->|No| Signal{"Errors up or latency up?"}
  Signal -->|Errors up| Deploy["Check recent deploy - roll back if it correlates"]
  Signal -->|Latency up| Saturation["Check saturation - raise max instances or check Dataflow backlog"]
  Deploy --> Capture["Capture trace exemplars for the postmortem"]
  Saturation --> Capture
```
*The first-five-minutes decision tree from the capstone runbook.*
