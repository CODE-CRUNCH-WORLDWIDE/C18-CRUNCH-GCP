...
```mermaid
flowchart TD
  A["Org level bindings"] --> B["Folder level bindings"]
  B --> C["Project level bindings"]
  C --> D["Resource level bindings"]
  D --> E["Effective policy equals union of all four levels"]
```
*Inheritance is one-directional and additive — nothing granted above can be subtracted below.*
...
```mermaid
flowchart LR
  P["Principal"] -->|"serviceAccountUser"| R["Attach SA to a new resource"]
  P -->|"serviceAccountTokenCreator"| T["Mint short lived token, become the SA from anywhere"]
```
*Same-sounding roles, different blast radius — attaching an SA is not the same as impersonating it.*
