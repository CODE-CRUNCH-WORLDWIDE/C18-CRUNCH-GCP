...
```mermaid
flowchart TD
  POOL["Workload identity pool"] --> PROV["Workload identity provider trusts one issuer"]
  PROV --> MAP["Attribute mapping turns claims into Google attributes"]
  PROV --> COND["Attribute condition gates which tokens are accepted"]
  MAP --> BIND["Binding grants workloadIdentityUser to a principalSet"]
  COND --> BIND
  BIND --> SA["Target service account"]
```
*The five WIF objects and how they chain from an external issuer to a GCP service account.*
...
sequenceDiagram
  ...
  participant GCP as GCP Resource

  GH->>STS: Present GitHub OIDC token
  STS->>STS: Validate signature and attribute condition
  STS-->>GH: Return federated access token
  GH->>IAMC: Exchange federated token for SA token
  IAMC-->>GH: Return one hour SA access token
  GH->>GCP: Call gcloud or Terraform as the SA
```
*No key file anywhere in the chain — every hop trades one short-lived token for another.*
