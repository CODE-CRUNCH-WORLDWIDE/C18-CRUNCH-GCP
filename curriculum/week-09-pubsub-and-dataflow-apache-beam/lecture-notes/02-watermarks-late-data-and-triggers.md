...
```mermaid
flowchart TD
  A["Window fires again"] --> B{"Accumulation mode"}
  B -->|"ACCUMULATING"| C["Firing has full result so far"]
  B -->|"DISCARDING"| D["Firing has only the new delta"]
  C --> E["Sink must upsert by window key"]
  D --> F["Sink must append the delta"]
```
*Accumulation mode decides what a firing contains, which decides what the sink must do with it.*
...
```mermaid
sequenceDiagram
  participant Src as Events
  participant Win as Window 12:03 to 12:04
  participant Sink as BigQuery Sink
  Src->>Win: Event A arrives at 12:03:05
  Win->>Sink: Early trigger upserts 100
  Src->>Win: Event B arrives at 12:03:50
  Win->>Sink: Early trigger upserts 150
  Note over Win: Watermark passes 12:04:00
  Win->>Sink: On-time trigger upserts 150
  Src->>Win: Late event C arrives at 12:08:00
  Win->>Sink: Late trigger upserts 175
  Note over Win: Allowed lateness expires at 12:14:20
  Win->>Win: Window state garbage collected
```
*The same window firing four times as early, on-time, and late triggers converge on the correct total.*
