...
```mermaid
flowchart TD
  A["Message published to topic"] --> B{"Subscription type"}
  B -->|"Pull"| C["Subscriber opens StreamingPull stream"]
  C --> D["Subscriber controls flow with FlowControl"]
  D --> E["Subscriber acks each message"]
  B -->|"Push"| F["Pub Sub sends HTTP POST to endpoint"]
  F --> G{"Endpoint response"}
  G -->|"2xx"| H["Pub Sub treats it as acked"]
  G -->|"Non-2xx or timeout"| I["Pub Sub nacks and retries with backoff"]
```
*Who initiates delivery and who controls the flow, for pull versus push.*
...
```mermaid
flowchart LR
  A["Message delivered to subscriber"] --> B{"Subscriber acks it"}
  B -->|"Yes"| C["Message removed as done"]
  B -->|"No"| D{"Delivery attempts reached max"}
  D -->|"Not yet"| A
  D -->|"Yes"| E["Pub Sub republishes to dead-letter topic"]
```
*A poison message loops through redelivery until it hits max_delivery_attempts, then moves to the DLQ.*
