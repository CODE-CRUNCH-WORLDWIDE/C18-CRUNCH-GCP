...
```mermaid
flowchart LR
  S0["Stage 0 Input - read columns, apply WHERE, partial aggregate"] --> Sh1["Shuffle"]
  Sh1 --> S1["Stage 1 Aggregate - combine partials"]
  S1 --> Sh2["Shuffle"]
  Sh2 --> S2["Stage 2 Output - format and return"]
```
*A simple group-by compiles into three stages; almost all the billed bytes are read in Stage 0.*
...
```mermaid
flowchart TD
  Table["trips_optimized table"] --> P1["Partition 2024-01-01"]
  Table --> P2["Partition 2024-01-02"]
  Table --> P3["Partition ..."]
  P1 --> B1["Cluster block - payment_type cash"]
  P1 --> B2["Cluster block - payment_type card"]
```
*Partitioning splits the table by date; clustering then sorts the blocks inside each partition by the cluster columns so a filter can skip blocks too.*
...
That is the toolkit. The exercises drill each piece; the challenge makes you put a dollar figure on the pricing-model decision; the mini-project assembles all of it on top of the Week 09 pipeline. Continue to the [exercises](../exercises/README.md).
