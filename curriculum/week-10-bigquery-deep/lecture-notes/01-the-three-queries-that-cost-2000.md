...
```mermaid
flowchart TD
  Q["Query submitted"] --> Dremel["Dremel orchestrator"]
  Dremel --> Slot1["Slot"]
  Dremel --> Slot2["Slot"]
  Dremel --> Slot3["Slot"]
  Slot1 --> Colossus["Colossus storage - Capacitor files"]
  Slot2 --> Colossus
  Slot3 --> Colossus
```
*Compute and storage are decoupled - any slot can read any file from Colossus, so cost tracks bytes read, not table size.*
...
```mermaid
flowchart LR
  T["Trips table - 50 million USD trips"] --> J["Join on currency only"]
  R["Fx rates table - 365 USD rows per year"] --> J
  J --> O["Output - 18.25 billion rows"]
```
*Joining on currency alone matches every trip against every rate row for that currency, exploding fifty million input rows into over eighteen billion output rows.*
