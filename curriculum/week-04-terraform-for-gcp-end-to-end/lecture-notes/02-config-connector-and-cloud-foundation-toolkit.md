...
```mermaid
flowchart LR
  CR["Custom Resource desired state"] --> Operator["In cluster operator watches"]
  Operator --> Compare["Compare desired vs live GCP resource"]
  Compare -->|Match| Wait["Wait a few minutes"]
  Compare -->|Diverged| Revert["Reconcile: revert drift automatically"]
  Wait --> Operator
  Revert --> Operator
```
*Config Connector reconciles on a continuous loop, so a console change is reverted within minutes instead of waiting for the next human-run plan.*
...
```mermaid
flowchart TD
  A["New GCP resource decision"] --> B{"Project org billing or foundational IAM?"}
  B -->|Yes| CFT["CFT on Terraform"]
  B -->|No| C{"Stateful and destructive change prone?"}
  C -->|Yes| TF["Terraform for the plan gate"]
  C -->|No| D{"Already on GKE GitOps for app adjacent resources?"}
  D -->|Yes| KCC["Config Connector"]
  D -->|No| E{"Your VPC or security opinions matter?"}
  E -->|Yes| TFH["Hand written Terraform modules"]
  E -->|No| KCC2["Config Connector for self serve"]
```
*Walk the decision table top to bottom; the first question that matches picks the tool.*
