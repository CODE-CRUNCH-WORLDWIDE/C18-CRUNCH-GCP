...
```mermaid
flowchart TD
  Root["envs terragrunt.hcl root config: remote_state and generate blocks"] --> Dev["envs dev terragrunt.hcl includes root"]
  Root --> Prod["envs prod terragrunt.hcl includes root"]
  Dev --> DevState["State prefix envs dev vpc terraform.tfstate"]
  Prod --> ProdState["State prefix envs prod vpc terraform.tfstate"]
```
*One root config generates the backend and provider blocks; each environment inherits it but writes to its own state prefix.*
...
```mermaid
flowchart LR
  PR["Open pull request"] --> CI1["CI runs fmt validate and plan"]
  CI1 --> Comment["Plan posted as PR comment"]
  Comment --> Human["Human reviews plan for forces replacement"]
  Human --> Merge["Merge to main"]
  Merge --> CI2["CI runs apply via Workload Identity Federation"]
```
*The plan-review loop: nothing reaches the cloud without a human reading the plan first.*
...
Leaving a `dev` environment billing overnight by accident is the most common way learners blow through the free trial. The teardown is part of the work, not an afterthought.
