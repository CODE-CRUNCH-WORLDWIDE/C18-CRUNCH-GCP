...
```mermaid
sequenceDiagram
  participant US as US ATM T1
  participant L as Split Leader us-central1
  participant EU as EU ATM T2
  US->>L: Withdraw 80
  L->>L: Read balance 100 then pick commit timestamp s1
  L->>L: Paxos write to majority
  Note over L: Commit-wait until TT.after s1
  EU->>L: Withdraw 50
  L--)EU: Blocked - row locked by T1
  L->>L: Commit-wait elapses then release lock - balance now 20
  EU->>L: Retry read balance 20
  L--)EU: Reject - 50 would overdraw
```
*TrueTime's commit-wait guarantees T2 always observes T1's committed balance, no matter which continent T2 started on.*
