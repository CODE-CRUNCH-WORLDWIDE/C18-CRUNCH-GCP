...
```mermaid
flowchart TD
  App["App in VPC"] --> PSC["PSC Endpoint - internal IP"]
  PSC --> Primary["Cloud SQL Primary - zone A"]
  Primary -->|"synchronous replication"| Standby["Standby - zone B"]
  Primary -->|"async replication"| SameRegion["Same-region Read Replica"]
  Primary -->|"async replication"| CrossRegion["Cross-region Read Replica"]
  CrossRegion -->|"promote on region loss"| DR["Standalone Primary - DR"]
```
*The four production properties assembled: PSC for private access, a synchronous standby for HA, and read replicas for read scaling and DR promotion.*
...
```mermaid
flowchart TD
  Start["Workload needs a database"] --> Q1{"Need horizontal write scale"}
  Q1 -->|"Yes"| Spanner["Spanner"]
  Q1 -->|"No"| Q2{"Need multi-region synchronous strong consistency"}
  Q2 -->|"Yes"| Spanner
  Q2 -->|"No"| Q3{"Need five nines with no failover"}
  Q3 -->|"Yes"| Spanner
  Q3 -->|"No"| Other["Cloud SQL or AlloyDB - Spanner would be theatre"]
```
*Spanner earns its cost on exactly three capabilities; absent all three it is a premium with nothing to show for it.*
