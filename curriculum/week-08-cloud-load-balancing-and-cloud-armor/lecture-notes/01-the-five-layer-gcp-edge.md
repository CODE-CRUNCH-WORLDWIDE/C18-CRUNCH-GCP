...
```mermaid
sequenceDiagram
  participant C as Client in Berlin
  ...
  D-->>C: Anycast IP Frankfurt edge
  C->>A: HTTPS request to slash api slash orders
  A->>A: Check rate limit and WAF rules
  A->>CDN: Allowed request
  CDN->>CDN: Cache miss private response
  CDN->>LB: Forward to URL map
  LB->>B: Route to Cloud Run backend
  B-->>C: Order data returned
```
*One request's path outside-in through all five layers, from DNS lookup to backend response.*
...
```mermaid
flowchart TD
  A["Cannot connect"] --> A1["DNS or forwarding rule"]
  B["TLS error"] --> B1["Certificate status"]
  C["403 response"] --> C1["Cloud Armor or IAP"]
  D["429 response"] --> D1["Rate limit rule"]
  E["Stale or wrong content"] --> E1["CDN cache key"]
  F["502 or 503 response"] --> F1["Backend health"]
```
*Symptom-to-layer map: the debugging tree that localizes an edge failure in under a minute.*
