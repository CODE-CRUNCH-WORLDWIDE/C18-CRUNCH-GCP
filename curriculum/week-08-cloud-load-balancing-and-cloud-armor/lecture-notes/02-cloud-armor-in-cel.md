...
```mermaid
flowchart TD
  A["Request arrives"] --> B{"Under threshold?"}
  B -- Yes --> C["conform action allow"]
  B -- No --> D{"Which action?"}
  D -- throttle --> E["Return 429 continuously"]
  D -- rate based ban --> F["Ban the key for ban duration"]
```
*Throttle keeps 429ing while over the line; rate-based ban drops the key entirely for the ban window.*
...
```mermaid
flowchart TD
  A["Priority 100 Office allow"] --> B["Priority 200 Geo deny"]
  B --> C["Priority 900 WAF SQLi deny"]
  C --> D["Priority 1000 Rate based ban"]
  D --> E["Default allow at max priority"]
```
*Evaluation order of the worked policy in section 2.5 — lowest priority number runs first.*
