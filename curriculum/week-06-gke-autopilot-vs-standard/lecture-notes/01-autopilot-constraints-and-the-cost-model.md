...
```mermaid
flowchart TD
  A["Needs a forbidden feature"] -->|Yes| Z["Choose Standard"]
  A -->|No| B["Pods far below the floor and many of them"]
  B -->|Yes| Y["Lean Standard - confirm with the numbers"]
  B -->|No| C["Pods medium sized with node slack"]
  C -->|Yes| X["Lean Autopilot - confirm with the numbers"]
  C -->|No| D["Weigh headcount for node operations"]
  D --> E["Check spot tolerance behind a PDB"]
  E --> F["Pick the cheaper total cost of ownership"]
```
*The five-question decision framework from section 1.10, run top to bottom.*
