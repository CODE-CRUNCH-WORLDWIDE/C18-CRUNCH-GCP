...
```mermaid
flowchart TD
  A["GKE cordons the node"] --> B["GKE drains the node"]
  B --> C["API server sends SIGTERM to each pod"]
  C --> D["Evict pod - check the PodDisruptionBudget"]
  D -->|Would breach minAvailable| E["Eviction refused - drain waits"]
  D -->|Allowed| F["Pod evicted and rescheduled"]
  E --> G["Wait for a replacement pod to become Ready"]
  G --> D
  F --> H["Node deleted once fully drained"]
```
*How GKE cordons, drains, and evicts pods while the PodDisruptionBudget paces the process.*
...
sequenceDiagram
  ...
  API->>EP: Pod marked for deletion
  EP->>EP: Remove pod from Service endpoints
  API->>Kubelet: Send SIGTERM
  Kubelet->>App: SIGTERM plus preStop hook
  App->>App: Finish in-flight requests
  App-->>Kubelet: Exit before grace period ends
  Kubelet->>App: SIGKILL if still running
```
*The graceful shutdown sequence that must complete before a drained pod is force-killed.*

Two things must be true for zero-downtime:
