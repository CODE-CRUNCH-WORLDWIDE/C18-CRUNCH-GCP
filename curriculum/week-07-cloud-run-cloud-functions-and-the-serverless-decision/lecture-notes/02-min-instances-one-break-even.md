...
```mermaid
flowchart LR
  A["Scheduling decision"] --> B["Image pull"]
  B --> C["Container start"]
  C --> D["App initialization"]
  D --> E["First request warmup"]
```
*The five phases a cold-served request pays for in sequence - image pull and app init are the ones you control.*
...
```mermaid
flowchart TD
  A["Measure the cold start penalty"] --> B["Shrink it first - slim image and lazy init"]
  B --> C["Estimate N cold from traffic shape"]
  C --> D{"SLO budget or business value"}
  D -->|SLO budget| E["N cold over N total exceeds error budget - min instances required"]
  D -->|Business value| F{"N cold times c cold greater than idle cost"}
  F -->|Yes| G["Set min instances to 1 or more - schedule if predictable"]
  F -->|No| H["Leave min instances at 0"]
```
*The end-to-end decision procedure for setting the min-instances floor on any service.*
