...
```mermaid
flowchart TD
  Start["Workload to place"] --> Duty{"Duty cycle"}
  Duty -->|Low - idle a lot| CR["Cloud Run - scale to zero"]
  Duty -->|High - busy around the clock| Fleet{"Existing GKE fleet"}
  Fleet -->|No| Numbers["Run the numbers - crossover RPS decides"]
  Fleet -->|Yes| GKE["GKE - bin pack onto fleet with tiny marginal cost"]
```
*The four-quadrant decision collapses to two questions: how idle is the service, and does a GKE fleet already exist.*
